...
;// CONCATENATED MODULE: ./src/search-vanilla.js
...
function initSearch() {
  var O_Search = document.querySelector('.O_SearchBar');
  var A_SearchInput = O_Search.querySelector('.A_Input');
  var A_Button = O_Search.querySelector('.A_Button');
  var requestText = getSearchRequest(); //посмотрели адрес поиска ???
...
function getSearchRequest() {
  ...
  var url = new URL(window.location.href); // Создается объект URLSearchParams, который содержит параметры поискового запроса из URL.

  var searchParams = new URLSearchParams(url.search); // Проверяем, есть ли параметр request в URL.
...
function setSearchRequest(requestText) {
  ...
  var url = window.location.href.split('?')[0]; // Устанавливаем новый URL с добавлением параметра request.
...
function SearchContent(requestText) {
  //запускаем рендер
  var contentItemsContainer = document.querySelector('.S_Content');
  contentItemsContainer.innerHTML = ''; // const requestText = requestText.toLowerCase()

  var contentItemIds = [];
  content.forEach(function (contentItem) {
    // обрабатываем контент. для каждой карточки нам нужно сотворить что-то страшное
    var nbspRegex = /[\u202F\u00A0]/gm; //ищет неразрывные пробелы (`\u202F` и `\u00A0`) в глобальном контексте (`gm`).

    var punctuationRegex = /[.,\/#!$%\^&\*;:{}=\-_`~()]/gm; //ищет знаки пунктуации в глобальном контексте.

    var title = contentItem.title.replaceAll(nbspRegex, ' ').replaceAll(punctuationRegex, '').toLowerCase();
    var description = contentItem.description.replaceAll(nbspRegex, ' ').replaceAll(punctuationRegex, '').toLowerCase();

    if (requestText.length >= 3) {
      if (title.includes(requestText.toLowerCase()) || description.includes(requestText.toLowerCase())) {
  ...
  if (contentItemIds.length > 0) {
    renderCardsByIds(contentItemsContainer, contentItemIds); //если не пусто, запускаем создание карточек по идентификатору
  } else {
...
function renderCardsByIds(container, ids) {
  ids = _toConsumableArray(new Set(ids)); // создаём множество уникальных значений из-за особенностей Set
...
function createContentCard(contentItemData) {
  ...
  contentItem.setAttribute('href', contentItemData.url); // const contentItemWrap = document.createElement('div')
